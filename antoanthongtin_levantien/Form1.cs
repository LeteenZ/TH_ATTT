namespace antoanthongtin_levantien
{
    public partial class Form1 : Form
    {
        ...
        public bool kiemTra()
        {
            if (txtBanro.Text.Length != 16 || txtKhoa.Text.Length != 16 || string.IsNullOrEmpty(txtBanro.Text) || string.IsNullOrEmpty(txtKhoa.Text))
                return true;
            return false;
        }
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                txtBanma.Text = "";
                string banrobd = txtBanro.Text.ToLower();
                string khoabd = txtKhoa.Text.ToLower();
                ...
                string banro = "";
                string banroip = "";
                foreach (char c in banrobd)
                {
                    banro += Convert.ToString(Convert.ToInt32(c.ToString(), 16), 2).PadLeft(4, '0');
                }

                for (int i = 0; i < 64; i++)
                {
                    banroip += banro[IP[i] - 1];
                }

                string sboxstring = "";
                string L = banroip.Substring(0,32);
                string R = banroip.Substring(32);
                string ER = "", P = "";
                ...
                txtBanma.Text = Convert.ToString(Convert.ToInt64(kq, 2), 16).ToUpper();
            }
            catch (Exception)
            {
                MessageBox.Show("Vui lòng nhập lại!");
